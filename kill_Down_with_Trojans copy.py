...
def DP(n, H, tile_types, tile_values):
    ...
    def helper(n, row, col, H, tile_types, tile_values, protection, multiplier):
        ...
        if row == n - 1 and col == n - 1:
            Memo[row][col] = H
            return Memo[row][col][protection][multiplier]
        ...
        if tile_types[row][col] == 0:
            ...
            Memo[row][col][protection][multiplier] = max(opt1 or opt2)
            ...
            Memo[row][col][protection][multiplier] = helper(n, row, col + 1, H, tile_types, tile_values, True, multiplier) \
                or helper(n, row + 1, col, H, tile_types, tile_values, True, multiplier)

        elif tile_types[row][col] == 3:
            Memo[row][col][protection][multiplier] = helper(n, row, col + 1, H, tile_types, tile_values, protection, True) \
                or helper(n, row + 1, col, H, tile_types, tile_values, protection, True)
        
        return Memo[row][col][protection][multiplier]

    return bool(helper(n, 0, 0, H, tile_types, tile_values, False, False))
